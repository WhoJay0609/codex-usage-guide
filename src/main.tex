...
\begin{itemize}
  ...
  \item \keypoint{Codex CLI}：适合终端优先、本地仓库内的快速读写和命令执行。
...
\begin{itemize}
  ...
  \item \keypoint{长流程、强审计、需要目标闭环}：使用 Goal 模式；本机入口是 \skill{goal-entry}，具体见最后一节。
\end{itemize}
...
\begin{itemize}
  ...
  \item \skill{goal-entry} 公开仓库：\url{https://github.com/WhoJay0609/codex-goal-entry}
\end{itemize}

\subsection{本机自写或本地维护 skills 备注}

本文提到的 skills 不是同一种来源。Codex app、Codex CLI、worktree、automation 等属于产品或官方能力；而 \skill{ask-research}、\skill{research-grill-me}、部分 Matt Pocock 风格 workflow、本机 Goal 入口和若干 \file{goal-*} skills 属于本机安装、本地维护或自写扩展。它们很有用，但不应被理解成 OpenAI 官方内置功能。
...
本指南最后一节只展开本机 \skill{goal-entry} 的具体实现；前面的 Goal 和 subagent 章节保持通用，方便没有安装这些本机 skills 的读者也能学习。
...
本节是全文最后一节，专门把前面的通用 Goal/Subagent 模型映射到本机实现。当前主入口是 \skill{goal-entry}。它是 router-only public entry，不把所有规则写在自己文件里，而是先用 resolver 分类，再路由到子 skill。公开版本仓库见：\url{https://github.com/WhoJay0609/codex-goal-entry}。

\subsection{本机实现备注}

\skill{goal-entry} 和相关 \skill{goal-*} skills 是本机维护的目标编排层，不是每个 Codex 安装环境都会自带的官方标准接口。阅读本节时应把它看成“一个可复制的本地实现案例”：通用原则是 Goal objective、readiness、context、dispatch、validation 和 closeout；具体脚本名、路径和路由规则以本机 \file{SKILL.md} 为准。
