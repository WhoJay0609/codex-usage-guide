...
\begin{itemize}
  ...
  \item \keypoint{原始需求混杂、需要先重构提示词}：使用 \skill{refine-user-prompt}，先展示任务合同，再按授权决定停止、回答、执行或创建 Goal。
  \item \keypoint{长流程、强审计、需要目标闭环}：优先使用通用 Goal 模式或可复现插件，并明确持久状态、验证和停止条件。
\end{itemize}
...
\begin{itemize}
  \item OpenAI Codex app 官方页面：\url{https://learn.chatgpt.com/docs/app}
  \item OpenAI Codex manual：\url{https://developers.openai.com/codex/codex-manual.md}
  \item OpenAI Codex permission modes：\url{https://learn.chatgpt.com/docs/permission-modes}
  \item OpenAI Codex Skills 与 Plugins：\url{https://learn.chatgpt.com/docs/skills-and-plugins}
  \item OpenAI Codex MCP：\url{https://learn.chatgpt.com/docs/extend/mcp}
  \item OpenAI GPT-5.6 prompting guidance：\url{https://developers.openai.com/api/docs/guides/prompt-guidance-gpt-5p6}
  \item OpenAI GPT-5.6 model guide：\url{https://developers.openai.com/api/docs/guides/latest-model}
  ...
  \item 第三方 \skill{refine-user-prompt}：\url{https://github.com/WhoJay0609/refine-user-prompt}
\end{itemize}

网页版本把每页的事实依据维护在 \file{data/site-manifest.json}，由生成器显示为“本页依据”；首页的最近更新与完整记录维护在 \file{data/changelog.json}。事实核验日期用于提示时效，不代替可追溯来源。第三方资料必须明确标成第三方上游，不能与 OpenAI 官方能力混写。

\subsection{第三方或本地维护 skills 备注}

本文提到的 skills 不是同一种来源。Codex app、worktree、automation、in-app browser、review 等属于产品或官方能力；Compound Engineering 属于第三方插件；\skill{refine-user-prompt} 是基于 OpenAI GPT-5.6 提示原则设计的第三方 skill；而 \skill{ask-research}、\skill{research-grill-me}、部分 Matt Pocock 风格 workflow 和若干 \file{goal-*} skills 属于本机安装、本地维护或自写扩展。第三方或本地 skill 都不应被理解成 OpenAI 官方内置功能。
...
本指南最后一节解读官方 GPT-5.6 prompting guidance，并把 \skill{refine-user-prompt} 作为第三方实现案例；前面的 Goal、subagent 和插件章节保持通用。
...
\begin{itemize}
  ...
  \item 提示词重构：见最后一节，再读 \skill{refine-user-prompt} 仓库中的 \file{SKILL.md}。
\end{itemize}
...
\section{GPT-5.6 提示词与 \skill{refine-user-prompt}}

OpenAI 的 GPT-5.6 prompting guidance 强调：先写清用户可见结果、关键约束、可用证据和完成标准，再删掉重复规则、无效示例和无关工具说明，让模型自己选择高效路径。官方在一组内部 coding-agent eval 中观察到，较精简的 system prompt 让得分约提高 10--15\%，总 token 下降 41--66\%，成本下降 33--67\%；这些数字只是方向性结果，迁移时仍要用自己的代表性任务验证。

官方原文见：\url{https://developers.openai.com/api/docs/guides/prompt-guidance-gpt-5p6}。本节的中文结构化解释属于本指南；\skill{refine-user-prompt} 则是第三方实现，不是 OpenAI 官方 skill，仓库见：\url{https://github.com/WhoJay0609/refine-user-prompt}。

\subsection{先保留什么，再删什么}

\begin{itemize}
  \item \keypoint{必须保留}：用户要得到的结果、明确事实与值、成功标准、停止条件、安全与权限边界、证据要求、输出格式和验证方式。
  \item \keypoint{优先删减}：同一规则的重复表达、不改变行为的过程旁白、无独特约束的示例、模型已稳定完成的脚手架、与当前任务无关的工具。
  \item \keypoint{先消除冲突}：GPT-5 系列会认真遵循 prompt contract；互相矛盾的强规则通常比缺少次要细节更不稳定。
  \item \keypoint{一次只改一组}：切换模型后先保留当前 reasoning effort 做基线，再逐组精简 prompt 或工具说明，并重跑同一批 eval。
\end{itemize}

\subsection{结果优先的 Codex 任务合同}

复杂任务可以从下面的短结构开始，只保留真正影响行为的栏目：
...
不要为了显得完整而固定十二步流程。真正不可变的安全顺序可以写成 must；搜索、提问、工具选择和迭代次数更适合写成决策规则。

\subsection{\skill{refine-user-prompt} 的四种模式}

这个 skill 必须先展示重构后的提示词，再根据原始请求的授权选择一种模式：

\begin{itemize}
  \item \skill{refine\_only}：只润色、只计划或明确不要执行；展示后停止。
  \item \skill{refine\_then\_answer}：有界、无副作用的解释、总结或审查；展示后直接回答。
  \item \skill{refine\_then\_execute}：普通、已授权的本地修改；展示后执行范围内工作并保留审批闸门。
  \item \skill{refine\_then\_create\_goal}：工作跨多阶段、需要恢复或长期状态，且执行已经获得授权；展示后再创建 Goal。
\end{itemize}

润色不会自动扩大权限。回答、解释、诊断和计划默认只读；外部写入、删除、购买、高成本任务或实质扩域仍需单独确认。

\subsection{模型与 reasoning effort 建议}

\skill{refine-user-prompt} 按任务歧义、依赖深度、工具和验证负担、错误后果、延迟与成本建议一个默认配置：

\begin{itemize}
  \item \skill{gpt-5.6-luna}：高效、例行、高吞吐或延迟敏感任务。
  \item \skill{gpt-5.6-terra}：日常 coding、分析和中等工具使用的平衡选择。
  \item \skill{gpt-5.6-sol}：复杂、高价值或高后果任务的旗舰质量选择；\skill{gpt-5.6} alias 路由到它。
\end{itemize}

通常从 \skill{medium} 开始，延迟敏感任务测试 \skill{low}；只有 eval 证明有明显收益时才升到 \skill{high} 或 \skill{xhigh}，\skill{max} 只留给最难的质量优先工作。这些都是建议，不会自动切换当前模型或 reasoning effort。
...
安装后开启一个新的 Codex 任务刷新 skill catalog。显式调用示例：
...
如果原始请求已经短、清楚且可直接执行，额外重构可能只增加往返。大量 prompt 自动评测、应用级 system prompt 迁移或训练优化器也应使用专门的 eval 流程，不要把单次任务润色 skill 当成自动优化框架。
