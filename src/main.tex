...
\begin{itemize}
  ...
  \item \keypoint{长流程、强审计、需要目标闭环}：使用 Goal 模式；本机入口见最后一节。
\end{itemize}
...
\begin{itemize}
  ...
  \item 学术流：读 \file{academic-research-suite/SKILL.md}、\file{ask-research/SKILL.md}。
...
本节是全文最后一节，专门把前面的通用 Goal/Subagent 模型映射到本机实现。当前主入口是 \skill{goal-entry}。它是 router-only public entry，不把所有规则写在自己文件里，而是先用 resolver 分类，再路由到子 skill。
...
\begin{lstlisting}
[$goal-entry](/home/hujie/.codex/skills/goal-entry/SKILL.md)
